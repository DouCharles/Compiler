...
Assignment_expression
	...
	|UnaryExpr assignment_op Assignment_expression	{$<s_val>$=$<s_val>1;	
...
Lor_expression
	...
	|Lor_expression LOR Land_expression		{ type_error($<s_val>1,"LOR",$<s_val>3);
CODEGEN("ior\n"); $<s_val>$ = "bool";}
;

Land_expression
	:Cmp_expression							{$<s_val>$ = $<s_val>1;}
	|Land_expression LAND Cmp_expression	{CODEGEN("iand\n"); $<s_val>$ = "bool";type_error($<s_val>1,"LAND",$<s_val>3);
}
;
...
DeclarationStmt
	:VAR VarID VarType					{
										...
										//	printf("222var\n");
											printf("error:%d: %s redeclared in this block. previous declaration at line %d\n",yylineno, $<s_val>2 ,lookup->lineno);
											g_has_error = true;
										}
										else
...
Condition
	:Expression	{if(strcmp($<s_val>1,"B")!=0)    {
				char *typee = NULL;
                if(sub_str = strstr($<s_val>1,"_id")){
					str2 = strdup($<s_val>1);
					sub_str = strstr(str2,"_id");
...
;
ForCondition
	:Expression	{CODEGEN("ifeq L_for_exit_%d\n",forLabelNum);}
;
...
//    printf("error=> %s %s %s \n",first,op,second);
	if(strcmp(op,"ifgt") == 0)
		op = "GTR";
	char *str_f = strdup(first);
	char *str_s = strdup(second);
    char *sub1 = NULL;
    char *sub2 = NULL;
    sub1 = strstr(str_f,"_id");
    sub2 = strstr(str_s,"_id");
    if(sub1) {
        sub1[0] = '\0';
		if (strcmp (str_f,"error") != 0){
		lookup = lookup_symbol(str_f);
		if(lookup->type[0] == 'I')
			str_f = "int32";
		else if (lookup->type[0] == 'F')
			str_f = "float32";
    ...
    if(sub2){
        sub2[0] = '\0';
		if(strcmp(str_s,"error") == 0){
		lookup = lookup_symbol(str_s);
		if(lookup->type[0] == 'I')
			str_s = "int32";
		else if (lookup->type[0] == 'F')
			str_s = "float32";
    ...
    if(strcmp(op,"REM")==0 && (strcmp(str_f,"float32") == 0 || strcmp(str_s,"float32") == 0)){
        char error_txt[1000];
        memset(error_txt, 0, 1000);
        strcat(error_txt,"invalid operation: (operator ");
        strcat(error_txt,op);
        strcat(error_txt," not defined on ");
        strcat(error_txt,strcmp(str_f,"float32") ==0? str_f:str_s);
        strcat(error_txt,")");
//      yyerror(error_txt);
        printf("error:%d: %s\n", yylineno+1, error_txt);
        return;
    }
    if((strcmp(op,"LAND")==0||strcmp(op,"LOR")==0) && (strcmp(str_f,"int32") == 0 || strcmp(str_s,"int32") == 0)){
        char error_txt[1000];
        memset(error_txt, 0, 1000);
        strcat(error_txt,"invalid operation: (operator ");
        strcat(error_txt,op);
        strcat(error_txt," not defined on ");
        strcat(error_txt,strcmp(str_f,"int32") ==0? str_f:str_s);
        strcat(error_txt,")");
//      yyerror(error_txt);
        printf("error:%d: %s\n", yylineno, error_txt);
        return;
    }
    if((strcmp(op,"GTR")==0) && (strcmp(str_f,str_s) != 0 )){
        char error_txt[1000];
        memset(error_txt, 0, 1000);
        strcat(error_txt,"invalid operation: ");
        strcat(error_txt,op);
        strcat(error_txt," (mismatched types ERROR and ");
        strcat(error_txt,str_s);
        strcat(error_txt,")");
//      yyerror(error_txt);
        printf("error:%d: %s\n", yylineno, error_txt);
        return;
    }
    if(strcmp(str_f,str_s) != 0){
        char error_txt[1000] ;
        memset(error_txt, 0, 1000);
        strcat(error_txt,"invalid operation: ");
        strcat(error_txt,op);
        strcat(error_txt," (mismatched types ");
        strcat(error_txt,str_f);
